function data=model_log_equation(log)
    if isfield(log,"litho")==0
        log=load_lithology(log);
    end
    datanew=zeros(length(log.Depth),1);
    a=[];
    ...
    len_a=length(a);
    prompt = a;
    dlgtitle = "Please the enter respective equation/expression" ;
    %dims = [1 35];
     ...
     for i=1:length(log.litho)
         ...
         for j=1:len_a
             if strcmpi(cell2mat(log.litho(i,3)),cell2mat(a(j)))
                 data=eval(answer{j});
                 datanew(indx1:indx2)=data(indx1:indx2);
